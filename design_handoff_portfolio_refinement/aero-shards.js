import { draw, effect, frame, init, sampler, surface, target, uniforms } from 'https://esm.sh/vgpu@latest';
const React = window.React;
const { useEffect, useRef, useState } = React;
const h = React.createElement;
if (!document.getElementById('aero-shards-css')) {
  const st = document.createElement('style');
  st.id = 'aero-shards-css';
  st.textContent = `.aero-shards{position:relative;width:100%;height:100%;overflow:hidden;pointer-events:none;isolation:isolate}
.aero-shards__canvas{position:absolute;inset:0;display:block;width:100%;height:100%;opacity:0;transition:opacity 650ms cubic-bezier(.22,1,.36,1);pointer-events:none}
.aero-shards[data-ready='true'] .aero-shards__canvas{opacity:1}
@media (prefers-reduced-motion:reduce){.aero-shards__canvas{transition:none}}`;
...
function AeroShards({
  ...
  return h('div', {
    ...
    'aria-hidden': 'true'
  }, h('canvas', { ref: canvasRef, className: 'aero-shards__canvas' }));
}
window.AeroShards = AeroShards;
